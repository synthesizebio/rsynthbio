...
```{r test-bulk-prediction}
test_that("predict_query for bulk prediction", {
  skip_if_not(check_api_key(),
              "SYNTHESIZE_API_KEY is not set")
  ...
  expect_true("sample_id" %in% colnames(result$metadata),
              info = "Metadata should have sample_id column")
  expect_true(nrow(result$metadata) > 0,
              info = "Should have at least one sample")

  # Check expression structure
  expect_s3_class(result$expression, "data.frame")
  expect_equal(nrow(result$expression), nrow(result$metadata),
               info = "Expression and metadata should have matching rows")
  expect_true(ncol(result$expression) > 1000,
              info = "Should have many genes (typically >10k)")
