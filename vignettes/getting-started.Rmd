...
Before using the Synthesize Bio API, you need to set up your API key.

To start using pysynthbio, first you need to have an account with synthesize.bio.
Go here to create and account and get your API key: https://app.synthesize.bio/

Once you have an account you can get [your API key from your profile](https://app.synthesize.bio/profile)

The best practice is to store your API key in your environment variables:

```{r auth, eval=FALSE}
# Set API key for the current R session
Sys.setenv(SYNTHESIZE_API_KEY = "your_api_key_here")

# For persistent storage, add to your .Renviron file
# file.edit("~/.Renviron")
# Add: SYNTHESIZE_API_KEY=your_api_key_here
```
...
The result contains two data frames:
- `metadata`: Information about each sample
- `expression`: Gene expression data (genes as columns, samples as rows)
...
# Get log-transformed data instead of raw counts
