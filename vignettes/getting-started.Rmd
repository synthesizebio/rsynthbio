...
The `predict_query()` function supports additional parameters to control model behavior:

```{r advanced-params, eval=FALSE}
...
- **`deterministic_latents`** (logical): If `TRUE`, the model uses the mean of each latent distribution (p(z|metadata) or q(z|x)) instead of sampling. This removes randomness from latent sampling and produces deterministic outputs for the same inputs. Useful for ensuring reproducibility in your analyses.

- **`total_count`** (integer): Library size used when converting predicted log CPM back to raw counts. Higher values scale counts up proportionally. If a reference expression is supplied (reference-conditioned endpoint) and `fixed_total_count` is `FALSE`, the model will ignore this value and use the reference's observed total counts instead.
